...
class HX711:
    ...
    def rawBytesToWeight(self, rawBytes=None, channel='A'):
        ...
        if hasattr(self, 'unit') and self.unit == 'ounce':
            weight *= 0.035274
    ...
    def tare(self):
        """
        Tares the scale by setting the current raw reading on Channel A as the offset.
        """
        if self.getChannel() != 'A':
            self.setChannel('A')
    
        rawBytes = self.readRawBytes()
        if rawBytes is None:
            raise ValueError("Failed to read raw data for tare function.")
    
        offsetValue = int.from_bytes(rawBytes, byteorder='big', signed=True)
    
        self.setOffset(offsetValue, channel='A')
    
        print(f"[INFO] Tare function completed. Offset set to: {offsetValue}")
        return offsetValue
    

    def setUnit(self, unit='gram'):
        """
        Sets the unit for weight measurements. Supported units: 'gram', 'ounce'.
        """
        if unit not in ['gram', 'ounce']:
            raise ValueError("Unsupported unit. Use 'gram' or 'ounce'.")
    
        self.unit = unit
        print(f"[INFO] Unit set to: {unit}")

    def getUnit(self):
        """
        Returns the currently set unit for weight measurements.
        """
        return self.unit
