import pygame, os
import utils.pitft_touchscreen as pitft_touchscreen
...
class PiTft:
    def __init__(
        self,
        rotation: int = -1,
        v2: bool = False if env["PIGAME_V2"] == "off" else True,
        allow_gpio: bool = True,
        invertx: bool = True if env["PIGAME_INVERTX"] == "on" else False,
        inverty: bool = True if env["PIGAME_INVERTY"] == "on" else False,
        swapxy: bool = True if env["PIGAME_SWAPXY"] == "on" else False,
        buttons=[
            False if env["PIGAME_BTN1"] == "off" else True,
            False if env["PIGAME_BTN2"] == "off" else True,
            False if env["PIGAME_BTN3"] == "off" else True,
            False if env["PIGAME_BTN4"] == "off" else True,
        ],
    ):
        self.use_gpio = (
            support_gpio and allow_gpio and not (os.getenv("PIGAME_GPIO") == "off")
        )
    ...
    def update(self):
        ...
        while not self.pitft.queue_empty():
            for r in self.pitft.get_event():
                e = {
                    "y": (r["x"] if r["x"] else self.cachedpos[0]),
                    "x": (r["y"] if r["y"] else self.cachedpos[1]),
                }
                ...
                t = (
                    MOUSEBUTTONUP
                    if r["touch"] == 0
                    else (MOUSEMOTION if self.pitft.button_down else MOUSEBUTTONDOWN)
                )
                if self.invertx:
                    e = {"x": 320 - e["x"], "y": e["y"]}
                    rel = (320 - rel[0], rel[1])
    ...
    def Button1Interrupt(self, callback=None, bouncetime=200):
        ...
        if self.__b1:
            GPIO.add_event_detect(
                self.__pin1, GPIO.FALLING, callback=callback, bouncetime=bouncetime
            )
    ...
    def Button2Interrupt(self, callback=None, bouncetime=200):
        ...
        if self.__b2:
            GPIO.add_event_detect(
                self.__pin2, GPIO.FALLING, callback=callback, bouncetime=bouncetime
            )
    ...
    def Button3Interrupt(self, callback=None, bouncetime=200):
        ...
        if self.__b3:
            GPIO.add_event_detect(
                self.__pin3, GPIO.FALLING, callback=callback, bouncetime=bouncetime
            )
    ...
    def Button4Interrupt(self, callback=None, bouncetime=200):
        ...
        if self.__b4:
            GPIO.add_event_detect(
                self.__pin4, GPIO.FALLING, callback=callback, bouncetime=bouncetime
            )
